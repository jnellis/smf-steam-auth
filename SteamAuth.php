		...
		try
		{
			...
			else
			{
				if ($openid->validate())
				{
					...
					$steamid = $matches[1];
// find a user acct that has a steam id specified in their profile
					$request = $smcFunc['db_query']('', '                          
						SELECT passwd, M.id_member, id_group, lngfile, is_activated, email_address, additional_groups, member_name, password_salt,
							openid_uri, passwd_flood
						FROM {db_prefix}members M
                        			INNER JOIN {db_prefix}themes T
                        			ON M.id_member = T.id_member
                        			WHERE T.variable = {string:steamidfield}
                      				AND T.value = {string:steamid}', 
						array(
							'steamid' => $steamid,  
							'steamidfield' => 'cust_steam_',
						)
					);  
					...
					// if that wasn't found then lookup the autogenerated name for this steamid.
					...
					// First time here with steam login. 
					if (empty($user_settings))
					{
